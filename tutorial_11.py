...
def hist2d_demo(image):
    hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)  # 换成HSV格式
    hist = cv.calcHist([image], [0, 1], None, [180, 256], [0, 180, 0, 256])  # 计算直方图
    # cv.imshow("hist2d", hist)
    plt.imshow(hist, interpolation='nearest')  # interpolation 是插值
...
def back_projection_demo():
    ...
    3. 设置适当的阈值对概率图像进行二值化
    """
    sample = cv.imread("D:/vcprojects/images/CrystalLiu3.jpg")
    target = cv.imread("D:/vcprojects/images/roi.png")
    ...
    roiHist = cv.calcHist([roi_hsv], [0, 1], None, [36, 48], [0, 180, 0, 256])  # 可通过调整直方图的bins个数调整效果 会更明显些 相当于细化了
    # 归一化：原始图像，结果图像，映射到结果图像中的最小值，最大值，归一化类型
    # cv.NORM_MINMAX对数组的所有值进行转化，使它们线性映射到最小值和最大值之间
    # 归一化后的图像便于显示，归一化后到0,255之间了
    cv.normalize(roiHist, roiHist, 0, 255, cv.NORM_MINMAX)
    dst = cv.calcBackProject([target_hsv], [0, 1], roiHist, [0, 100, 0, 256], 1)  # 注意一定要用HSV色彩空间的
